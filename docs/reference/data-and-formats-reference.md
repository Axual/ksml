...
**Syntax:**
```yaml
# With a local schema file (loads <schema_name>.avsc from schemaDirectory):
valueType: avro:<schema_name>

# With schema registry lookup (fetches the latest schema from the registry using the topic name):
...
streams:
  sensor_readings:
    topic: sensor-data
    keyType: string
    valueType: confluent_avro     # Fetches latest schema for subject "sensor-data-value" from registry
```

When using schema registry lookup (no schema name), KSML derives the schema registry subject from the topic name using the `TopicNameStrategy`: `{topic}-value` for value types and `{topic}-key` for key types. This is useful when schemas are dynamically generated by external systems (e.g., Kafka Connect) and only exist in the schema registry.

!!! note
    Schema registry lookup requires a schema registry to be configured in `ksml-runner.yaml` under the `schemaRegistries` section, and the notation must reference it. See the [Configuration Reference](configuration-reference.md) for details.
...
When no schema name is specified and the notation is backed by a schema registry (e.g., `confluent_avro`), KSML fetches the latest schema from the registry at startup using the topic name:
