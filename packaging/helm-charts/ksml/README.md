...
| ksmlRunnerConfig.storageDirectory                  | Directory used for state storage. This should only be changed if an external volume mount is used.                                                                                                                                  | <code>'/ksml-store/data'</code>                                                                |
| ksmlRunnerConfig.createStorageDirectory            | Create the storage directory if it does not exist.                                                                                                                                                                                  | <code>'true'</code>                                                                            |
...
| volumeClaimTemplates                               | A list of volume claim templates to use by KSML. Set this if the application needs extra persistent volumes storage.                                                                                                                |                                                                                                |
| store.spec                                         | The specification for the volumeClaimTemplate use as state storage. Set this if the application needs state storage. If not set a 10 megabyte empty dir is used for storage.                                                        |                                                                                                |
