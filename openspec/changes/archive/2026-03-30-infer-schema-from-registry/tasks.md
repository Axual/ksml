...
- [x] 1. Create `UnresolvedType` marker class
...
- [x] 2. Add `fetchRemoteSchema(String subject)` to `Notation` interface
  - Default method returning `null`
  - No changes needed in existing notation implementations

- [x] 3. Implement `fetchRemoteSchema` in Confluent Avro notation
...
- [x] 4. Modify `UserTypeParser.parseNotationWithOrWithoutSchema()`
...
- [x] 5. Add resolution logic in `StreamDefinitionParser`, `TopicDefinitionParser`, `TableDefinitionParser`, and `GlobalTableDefinitionParser`
...
- [x] 6. Add caching in `SchemaLibrary` for registry-fetched schemas
