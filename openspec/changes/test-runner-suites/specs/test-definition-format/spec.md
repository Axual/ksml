...
**Reason**: Replaced by stream-referenced produce blocks. The same per-message fields (`key`, `value`, `timestamp`) and topic-level fields (`keyType`, `valueType`) are still supported, but `keyType` and `valueType` move into the suite-level `streams:` map and the produce block now references the stream by name via `to:` instead of carrying `topic`/`keyType`/`valueType` directly.
**Migration**: Replace the per-block `topic: foo`, `keyType: x`, `valueType: y` with a `streams:` entry under a logical name (`my_stream: { topic: foo, keyType: x, valueType: y }`) and change the produce block to reference it: `to: my_stream`. The `messages:` list is unchanged.

### Requirement: Generator-based test data
**Reason**: The generator block itself is unchanged, but it now lives under a produce block that references a stream via `to:` rather than carrying its own `topic`/`keyType`/`valueType`. Re-stated under "Stream-referenced produce block with generator" below.
**Migration**: Same as for inline messages — move `topic`/`keyType`/`valueType` into a `streams:` entry and replace with `to: <stream_key>` on the produce block.

### Requirement: Assertion blocks with output records
**Reason**: Replaced by stream-referenced assertion blocks. The injected `records` variable and its semantics are unchanged, but the assert block now identifies the source via `on: <stream_key>` instead of `topic: <topic_name>`.
**Migration**: Add a `streams:` entry for the output topic if not already present, and change `topic: foo` in the assert block to `on: <stream_key>`.
