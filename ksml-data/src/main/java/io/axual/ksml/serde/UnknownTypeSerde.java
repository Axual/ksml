...
public class UnknownTypeSerde implements Serde<DataObject> {
    ...
    public Serializer<DataObject> serializer() {
        return (topic, object) -> {
            throw new KSMLDataException("Can not serialize data dataType \"" + type + "\" from topic " + topic);
        };
    }
    ...
    public Deserializer<DataObject> deserializer() {
        return (topic, bytes) -> {
            throw new KSMLDataException("Can not deserialize data dataType \"" + type + "\" from topic " + topic);
        };
    }
}
