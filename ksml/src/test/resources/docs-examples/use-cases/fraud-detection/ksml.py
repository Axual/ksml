...
if TYPE_CHECKING:
    ...
    class ValueAndTimestamp(Protocol):
        """Value and timestamp record returned by TimestampedKeyValueStore and TimestampedWindowStore"""
    ...
    class TimestampedWindowStore(StateStore, Protocol):
        ...
        def put(self, key: Any, value: ValueAndTimestamp, windowStartTimestamp: int) -> None: ...
        def put(self, key: Any, value: Any, windowStartTimestamp: int, timestamp: int) -> None: ...
        def fetch(self, key: Any, time: int) -> Optional[ValueAndTimestamp]: ...
        def fetch(self, key: Any, timeFrom: int, timeTo: int) -> Iterator[Any]: ...
        def backwardFetch(self, key: Any, timeFrom: int, timeTo: int) -> Iterator[Any]: ...
        def fetchAll(self, timeFrom: int, timeTo: int) -> Iterator[Any]: ...
        def backwardFetchAll(self, timeFrom: int, timeTo: int) -> Iterator[Any]: ...
    ...
    class ValueAndTimestampFactory(Protocol):
        """Factory for creating ValueAndTimestamp instances"""

        def make(self, value: Any, timestamp: int) -> ValueAndTimestamp: ...

    # KSML runtime injected variables
    log: PythonLogger
    metrics: MetricsBridge
    stores: Dict[str, Union[StateStore, KeyValueStore, SessionStore, WindowStore, VersionedKeyValueStore, TimestampedKeyValueStore, TimestampedWindowStore, Any]]
    valueAndTimestamp: ValueAndTimestampFactory
else:
    ...
    TimestampedKeyValueStore = java.type('io.axual.ksml.store.TimestampedKeyValueStoreProxy')
    TimestampedWindowStore = java.type('io.axual.ksml.store.TimestampedWindowStoreProxy')
    ValueAndTimestamp = java.type('io.axual.ksml.store.ValueAndTimestampProxy')
    ValueAndTimestampFactory = java.type('io.axual.ksml.store.ValueAndTimestampFactory')
