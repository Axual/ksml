...
class TransformKeyOperationTest {

    @Test
    @DisplayName("transform key on a stream returns a KStream and processes records via a processor supplier")
    ...
    void applyToStreamProcessesRecords() {
        ...
        final var operation = new TransformKeyOperation(operationConfig("mapKey"), keyTransformer());

        assertThat(operation.apply(input, mockContext())).isInstanceOf(KStreamWrapper.class);
        verify(stream).process(any(ProcessorSupplier.class), any(Named.class));
    }
}
